...
def build_title_or_abst_matrix(data_file, ngram, df, tfidf):
    ...
    if tfidf == False:
        vectorizer = CountVectorizer(
            min_df=df, stop_words=STOP_WORDS, ngram_range=(1, ngram)
        )
    else:
...
def build_meta_matrix(data_file, ngram, df, tfidf):
    ...
    if tfidf == False:
        journal_vect = CountVectorizer(
            min_df=df, stop_words=STOP_WORDS, ngram_range=(1, ngram)
        )
    else:
        journal_vect = TfidfVectorizer(
            min_df=df, stop_words=STOP_WORDS, ngram_range=(1, ngram)
        )
    ...
    meta_matrix = np.column_stack(
        (year_matrix, author_matrix, CitCount_matrix, journal_matrix)
    )
...
def build_matrices_old(file, ngram, df, tfidf, class_type):
    """
    ...
    if class_type == "T":
        X_matrix = build_title_or_abst_matrix(
            ML_PATH / (file + TITLES_FILE), ngram, df, tfidf
        )

    elif class_type == "A":
        X_matrix = build_title_or_abst_matrix(
            ML_PATH / (file + ABSTRACTS_FILE), ngram, df, tfidf
        )
        ...
        title_matrix = build_title_or_abst_matrix(
            ML_PATH / (file + TITLES_FILE), ngram, df, tfidf
        )
        meta_matrix = build_meta_matrix(
            ML_PATH / (file + METADATA_FILE), ngram, df, tfidf
        )
...
def build_matrices(file, ngram, df, tfidf, class_type):
    ...
    if "T" in class_type:
        ...
        partial_matrices.append(X_matrix)
    ...
    if len(partial_matrices) == 1:
        X_matrix = partial_matrices[0]

    else:
        X_matrix = np.column_stack(tuple(partial_matrices))
